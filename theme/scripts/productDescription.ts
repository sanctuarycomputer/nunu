  ...
  const ProductDescription = {
    ...
    init() {
      ...
      const tabsWrapper: HTMLElement = document.querySelector(
        ProductDescription.SELECTORS.tabsWrapper
      );
    ...
    toggleActiveStyling(isActive: boolean, tab: HTMLElement) {
      if (isActive) {
        tab.classList.add(ProductDescription.CLASSES.activeTab);
      } else {
        tab.classList.remove(ProductDescription.CLASSES.activeTab);
      }
    },

    scrollHandler(
      tabs: HTMLElement[],
      tabbedContent: HTMLElement[],
      height: number
    ) {
      tabbedContent.forEach((elem, index) => {
        ...
        if (index == 0) {
          ...
          isActive =
            window.scrollY >=
            tabbedContent[index - 1].offsetTop +
              tabbedContent[index - 1].offsetHeight -
              height;
        } else {
          isActive =
            window.scrollY >= offsetTop - STICKY_HEIGHT_BUFFER &&
            window.scrollY < offsetBottom - height;
        }
